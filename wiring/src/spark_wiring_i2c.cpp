...
ssize_t TwoWire::requestFrom(uint8_t address, size_t quantity, uint8_t sendStop)
{
  return requestFrom(WireTransmission(address).quantity(quantity).stop(sendStop));
}

ssize_t TwoWire::requestFrom(uint8_t address, size_t quantity)
{
  return requestFrom(address, quantity, (uint8_t)true);
}

ssize_t TwoWire::requestFrom(const WireTransmission& transfer) {
